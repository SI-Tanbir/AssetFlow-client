...
const Dashboard_layout = () => {
  ...
  const { email,signOutFirebase } = useContext(MyContext);
  const [admin, setAdmin] = useState(false);
  const [loading, setLoading] = useState(true);
  // console.log(email)
  useEffect(() => {
    baseUrl.post("/admin", { email: email }).then((res) => {
          ...
          <ul className="menu bg-base-200 text-base-content min-h-full w-80 p-4">
            ...
            {admin ? (
              <>
                <li
                  className={`p-2 ${
                    activeItem === "adminHome" ? "bg-purple-600 text-white" : ""
                  }`}
                ...
                <li
                  className={`p-2 ${
                    activeItem === "assetList" ? "bg-purple-600 text-white" : ""
                  }`}
                ...
                <li
                  className={`p-2 ${
                    activeItem === "addAsset" ? "bg-purple-600 text-white" : ""
                  }`}
                  onClick={() => setActiveItem("addAsset")}
                >
                  <NavLink to={"/dashboard/add-assets"}>
              ...
              <>
                <li
                  className={`p-2 ${
                    activeItem === "home" ? "bg-purple-500 text-white" : ""
                  }`}
                ...
                <li
                  className={` p-2 ${
                    activeItem === "myAssets" ? "bg-purple-600 text" : ""
                  }`}
                ...
                <li
                  className={`p-2 ${
                    activeItem === "myTeam" ? "bg-purple-600 text-white" : ""
                  }`}
...
<button onClick={handleLogout} className="flex absolute top-[90%] text-3xl hover:bg-purple-500">
